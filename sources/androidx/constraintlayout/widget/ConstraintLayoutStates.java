...
public class ConstraintLayoutStates {
    ...
    private void parseConstraintSet(Context context, XmlPullParser parser) {
        ...
        for (int i = 0; i < count; i++) {
            if (DBHelper.CONTACTS_COLUMN_ID.equals(parser.getAttributeName(i))) {
                String s = parser.getAttributeValue(i);
                int id = -1;
                if (s.contains("/")) {
                    id = context.getResources().getIdentifier(s.substring(s.indexOf(47) + 1), DBHelper.CONTACTS_COLUMN_ID, context.getPackageName());
                }
                if (id == -1) {
